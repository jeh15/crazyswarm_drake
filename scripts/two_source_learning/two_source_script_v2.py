...
def main(argv=None):
    ...
    # Reference -> Adversary
    builder.Connect(
        reference.get_output_port(driver_reference.linear_output),
    ...
    # Learning Framework:
    fig_regression, ax_regression = plt.subplots(4)
    fig_regression.tight_layout(pad=2.5)

    t_data_plot, = ax_regression[0].plot([], [], color='black', marker='.', linestyle='None')
    a_data_plot, = ax_regression[2].plot([], [], color='black', marker='.', linestyle='None')
    t_fp, = ax_regression[0].plot([], [], color='red')
    t_ls, = ax_regression[1].plot([], [], color='red')
    a_fp, = ax_regression[2].plot([], [], color='red')
    a_ls, = ax_regression[3].plot([], [], color='red')
    ...
    # FP Plot:
    ax_regression[2].set_xlim([-1, 3])  # X Lim
    ax_regression[2].set_ylim([-1, 2])  # Y Lim
    ax_regression[2].set_xlabel('delta')  # X Label
    ax_regression[2].set_ylabel('r(delta)')  # Y Label
    # LS Plot:
    ax_regression[3].set_xlim([-1, 3])  # X Lim
    ax_regression[3].set_ylim([-2, 1])  # Y Lim
    ax_regression[3].set_xlabel('delta')  # X Label
    ax_regression[3].set_ylabel('s(delta)')  # Y Label
    ...
    with writerObj.saving(fig_regression, video_title+".mp4", dpi):
        for i in range(0, simulation_size):
            ...
            for _ in range(0, int(sample_rate)):
                # Plot FP:
                t_data_plot.set_data(tracker_data_history[i][0, :], tracker_data_history[i][1, :])
                t_fp.set_data(tracker_fp_history[i][0, :], tracker_fp_history[i][1, :])
                a_data_plot.set_data(avoidance_data_history[i][0, :], avoidance_data_history[i][1, :])
                a_fp.set_data(avoidance_fp_history[i][0, :], avoidance_fp_history[i][1, :])
                # Plot LS:
                t_ls.set_data(tracker_ls_history[i][0, :], tracker_ls_history[i][1, :])
                a_ls.set_data(avoidance_ls_history[i][0, :], avoidance_ls_history[i][1, :])
                # Update Drawing:
