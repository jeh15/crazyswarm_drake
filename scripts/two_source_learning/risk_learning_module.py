...
class RiskLearning(LeafSystem):
    def __init__(self, config: ml_collections.ConfigDict(), failure_radius: float):
        ...
        self._failure_radius = failure_radius
        ...
        def on_initialize(context, event):
            ...
            initial_data = self.evaluate(self=self, context=context)
        ...
        def periodic_update_event(context, event):
            current_data = self.evaluate(self=self, context=context)
    ...
    def evaluate(self, context) -> np.ndarray:
        # Method defined through extension
        pass
