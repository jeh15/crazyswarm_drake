...
def get_config() -> ml_collections.ConfigDict():
    ...
    # Future Samples:
    config.sample_factor = 2
    config.sample_nodes = int(np.floor((config.nodes - 1) / config.sample_factor))
    config.samples = (
        np.arange(config.motion_planner_rate, config.time_horizon, config.dt, dtype=float)
    )[:config.sample_nodes]
    # Spline Parameters:
...
def main(argv=None):
    ...
    builder.Connect(
        parser.get_output_port(driver_parser.control_output),
        crazyswarm.get_input_port(0),
    )

    # Parser Previous Trajectory -> Motion Planner Continuity Cost:
    builder.Connect(
        parser.get_output_port(driver_parser.previous_trajectory_output),
        planner.get_input_port(driver_planner.continuity_input)
    )
